...
export const apiSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getTabsData: builder.query({
      queryFn: async () => {
        try {
          const response = await axios.get('http://localhost/microdose-april-26/wp-json/wp/v2/posts', {
            params: {
              categories: 37,
              per_page: 100,
              _embed: true,
    ...
    getMenuItemsHeader: builder.query({
      queryFn: async () => {
        try {
          const response = await axios.get('http://localhost/microdose-april-26/wp-json/wp-api-menus/v2/menus/6', {
            params: {
              // Add timestamp to force fresh data on page load
              _t: Date.now()
            }
    ...
    getMenuItemsFooter: builder.query({
      queryFn: async () => {
        try {
          const response = await axios.get('http://localhost/microdose-april-26/wp-json/wp-api-menus/v2/menus/14', {
            params: {
              // Add timestamp to force fresh data on page load
              _t: Date.now()
            }
    ...
    getPageBySlug: builder.query({
      queryFn: async (slug) => {
        try {
          const res = await axios.get(`http://localhost/microdose-april-26/wp-json/wp/v2/pages`, {
            params: {
              slug: slug,
              // Add timestamp to force fresh data on page load
              _t: Date.now()
